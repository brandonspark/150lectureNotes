...
\begin{frame}[plain]
  \begin{center}
    \huge \intermedium{1 - The Philosophy of Functional Programming}
  \end{center}
\end{frame}
...
\begin{frame}[fragile]
  ...
  This demonstrates what we call \textbf{state}.
...
%\setbeamercolor{normal text}{fg=bgColor}
%\usebeamercolor[fg]{normal text}
\begin{frame}[plain]
  \begin{center}
    \huge \intermedium{2 - Types, Expressions, Values}
  \end{center}
\end{frame}
%}
...
\begin{frame}[plain]
  ...
  \defBox{\, We use the $\stepsTo$ symbol to denote \textbf{stepping} (or \textbf{reducing}) of expressions,
...
\begin{frame}[plain]
  ...
  We call the previous slide the \textbf{computation trace} of the expression \code{(2 + 3) * 4}. \\ 
  ...
  \defBox{\, An \textbf{expression} is the building block of an SML program. These 
  may or may not step to another expression.}

  \pause
  \vspace{\fill}

  \defBox{\, A \textbf{value} is a \textbf{final answer}, that cannot be simplified further.}
...
\begin{frame}[plain]
  ...
  \defBox{\, A \textbf{type} is a specification of the behavior of a piece of code. It 
...
\begin{frame}[plain]
  ...
  So \code{"1" ^ 50} does not have a type, and we say it is an \textbf{ill-typed expression}.
\end{frame}
...
\begin{frame}[plain]
  ...
  \textbf{type-checks}, or is \textbf{well-typed}.
  }
