...
\begin{frame}[fragile]
  ...
  \begin{center}
    \includegraphics[scale=0.25]{bucket_0}
  \end{center}
...
\begin{frame}[fragile]
  ...
  \begin{center} \includegraphics[scale=0.25]{bucket_0_and_1} \end{center}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \pause
  \begin{center} \includegraphics[scale=0.25]{bucket_n} \end{center}
...
\begin{frame}[fragile]
  ...
  \pause
  \begin{center} \includegraphics[scale=0.15]{bucket_nil} \end{center}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \pause
  \begin{center} \includegraphics[scale=0.15]{bucket_singletons} \end{center}
...
\begin{frame}[fragile]
  ...
  \begin{center}
  \includegraphics[scale=0.4]{tail_recursion}
  \end{center}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \pause
  We already saw \code{length} was not tail recursive.
  ...
  \pause
  Similarly, \code{fact} ends up not being tail recursive.
  ...
  \pause
  Let's call \code{isEmpty} "vacuously" tail recursive.
...
\sectionSlide{5}{Case Study: \code{length} and \code{tlength}}

\begin{frame}[fragile]
  \frametitle{But Is It Right?}

  We've written \code{tlength}, which computes the same thing as
  \code{length}, just more efficiently\footnote{\textit{\color{blue}[citation needed]}}.

  \pause
  \vspace{\fill}

  But this is a claim that deserves a proof! Our \code{length} function is our
  \term{reference implementation}, and we'd like to show that \code{tlength}
  agrees with it.

  \pause
  \vspace{\fill}

  \thmBox{}\, For all \code{L : int list}, \code{tlength (L, 0)} $\eeq$ \code{length L}.

  \pause
  \vspace{\fill}

  Let's try to prove this by structural induction on \code{L}.
\end{frame}

\begin{frame}[fragile]
  \frametitle{A Failed Attempt}
  ...
  \bcBox{\, \code{L = []}}
  \begin{align*}
    \code{tlength ([], 0)} &\stepsTo \code{0} & \text{(clause 1 of \code{tlength})} \\
                           &\eeq \code{length []} & \text{(clause 1 of \code{length})}
  \end{align*}

  \pause
  \vspace{-5pt}

  \ihBox{}{\, Case: \code{L = xs}. Assume \code{tlength (xs, 0)} $\eeq$ \code{length xs}.}

  \vspace{\fill}

  \isBox{}{\, Case: \code{L = x::xs}. WTS \code{tlength (x::xs, 0)} $\eeq$ \code{length (x::xs)}.}
  \pause
  \begin{align*}
    \code{tlength (x::xs, 0)} &\stepsTo \code{tlength (xs, 1 + 0)} & \text{(clause 2 of \code{tlength})} \\
                              &\stepsTo \code{tlength (xs, 1)}     & \text{(math)}
  \end{align*}

  \pause
  \vspace{-3pt}

  We're stuck! Our inductive hypothesis tells us about \code{tlength (xs, 0)}, but
  we have \code{tlength (xs, 1)}. \textbf{The IH does not apply.}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Strengthening the Theorem}

  The issue is that the accumulator \textit{changes} on every recursive call. Our
  IH only tells us what happens at \code{0}, but the recursive call passes a
  different value.

  \pause
  \vspace{\fill}

  The fix is to prove a \term{stronger} theorem -- one which holds for \textit{any}
  starting value of the accumulator.

  \pause
  \vspace{\fill}

  \thmBox{}\, For all \code{L : int list} and all \code{n : int},
  \code{tlength (L, n)} $\eeq$ \code{length L + n}.

  \pause
  \vspace{\fill}

  Notice that our original theorem is just the special case where \code{n = 0},
  since \code{length L + 0} $\hookrightarrow$ \code{length L}.
\end{frame}

\begin{frame}[fragile]
  \frametitle{The Proof, Take Two}
  ...
  \bcBox{\, \code{L = []}}
  \begin{align*}
    \code{tlength ([], n)} &\stepsTo \code{n}              & \text{(clause 1 of \code{tlength})} \\
                           &\eeq \code{0 + n}              & \text{(math)} \\
                           &\eeq \code{length [] + n}      & \text{(clause 1 of \code{length})}
  \end{align*}

  \pause
  \vspace{\fill}

  \ihBox{}{\, Case: \code{L = xs}. Assume that for all \code{n : int},
  \code{tlength (xs, n)} $\eeq$ \code{length xs + n}.}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \isBox{}{\, Case: \code{L = x::xs}. We'd like to show that for all \code{n : int},
  \code{tlength (x::xs, n)} $\eeq$ \code{length (x::xs) + n}.}

  \pause
  \begin{align*}
    \code{tlength (x::xs, n)} &\stepsTo \code{tlength (xs, 1 + n)}   & \text{(clause 2 of \code{tlength})} \\
                              &\eeq \code{length xs + (1 + n)} & \text{(IH, but see warning)} \\
                              &\eeq \code{(1 + length xs) + n}       & \text{(associativity)} \\
                              &\eeq \code{length (x::xs) + n}        & \text{(clause 2 of \code{length})}
  \end{align*}

  \customColorBox{red}{white}{Warning}{\, There is a technicality here which we will discuss in the next lecture.
  When we apply the IH, we are actually going to also need to know that \code{+} is total.}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Quantifiers Matter}

  Look carefully at the inductive hypothesis we used:

  \pause
  \vspace{\fill}

  \customBox{IH}{\, For all \code{n : int}, \code{tlength (xs, n)} $\eeq$ \code{length xs + n}.}

  \pause
  \vspace{\fill}

  This is crucially different from the IH in our failed attempt! There, the IH only
  spoke about a \textit{single} value of the accumulator, \code{0}. Here, the IH
  quantifies over \textit{all} \code{n}, which is what allowed us to instantiate it
  at \code{1 + n}.

  \pause
  \vspace{\fill}

  This is why strengthening the theorem worked. A stronger theorem gives us a
  \textbf{stronger inductive hypothesis} -- and sometimes that's exactly what we need.
\end{frame}

\begin{frame}[fragile]
  \frametitle{The Real Theorem is a Corollary}

  Now, the result we actually wanted falls right out:

  \pause
  \vspace{5pt}

  \thmBox{}\, For all \code{L : int list}, \code{tlength (L, 0)} $\eeq$ \code{length L + 0}.

  \pause
  \vspace{\fill}

  By the powers invested in us by the arithmetic gods, we know that this is really
...
\begin{frame}[fragile]
  \frametitle{A Lesson About Accumulators}

  This is a pattern which shows up a lot when doing mathematics.

  \pause
  \vspace{\fill}

  Sometimes, the theorem you really want is a \term{corollary} of a stronger statement
  -- one which quantifies over all possible values of the accumulator.

  \pause
  \vspace{\fill}

  Why? Because the accumulator changes on every recursive call. If your IH only
  tells you about one specific accumulator value, it won't match the recursive
  call's argument, and you'll get stuck -- just like we did the first time.
\end{frame}
