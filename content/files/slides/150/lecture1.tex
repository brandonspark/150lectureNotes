...
%     fun qsort [] = []
%       | qsort (x::xs) =
%           let val (lo, hi) = List.partition (fn y => y < x) xs
%           in qsort lo @ [x] @ qsort hi end
%   \end{lstlisting}
...
%   \begin{center}
%     This is an example of the \textbf{quicksort} algorithm, in a language called Standard ML.
%   \end{center}
...
%   \begin{center}
%     Right now, this looks like nonsense.
%   \end{center}
% \end{frame}
...
\begin{frame}[fragile]
  ...
  I'm an alum who graduated four years ago\footnote{Wow, time passes quick.} with a degree in computer science.

  \pause
  \vspace{\fill}

  I TA'd 150 for seven semesters while I was at CMU, and I also taught this course
  three years ago, and then retired from teaching. It is a bad sign that I am back here now.

  \pause
  \vspace{\fill}

  I currently work a full-time job as a software engineer at a security company called
...
\begin{frame}[fragile]
    ...
    \begin{center}
      \includegraphics[scale=0.04]{me_new.jpg}
    \end{center}
    \begin{center}
      {\Large Brandon Wu} \\
      \vspace{1pt}
    ...
    \begin{center}\begin{tabular}{c c c c c c c c }
      Alessandro & Andrew & Angie & Arnim & Veer & Vincent \\
      \includegraphics[width=0.1\columnwidth]{alessandro_3x4.jpg} &
      \includegraphics[width=0.1\columnwidth]{andrew_3x4.jpg} &
      \includegraphics[width=0.1\columnwidth]{angie_3x4.jpg} &
      \includegraphics[width=0.1\columnwidth]{arnim_3x4.jpg} &
      \includegraphics[width=0.1\columnwidth]{veer_3x4.jpg} &
      \includegraphics[width=0.1\columnwidth]{vincent_3x4.jpg} \\
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Quizzes most weeks on Fridays
    ...
    \item Slides, course policy, and other information at the {\color{blue}\href{http://www.cs.cmu.edu/~15150/}{course website here}}\footnote{As of May 12th, still under construction.}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{minipage}{0.4\textwidth}
    \includegraphics[scale=0.1]{farnam_grades.jpg}
  \end{minipage}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \end{minipage}
  \begin{minipage}{0.4\textwidth}
    \includegraphics[scale=0.1]{farnam_grades_v2.jpg}
  \end{minipage}
\end{frame}
...
\begin{frame}[fragile]
  ...
  One practice we are instituting this semester is \textbf{quizzes}.

  \pause
  \vspace{\fill}

  Midterms are good for synchronously testing your understanding of the material, but they
  come too infrequently to be a good temperature check.

  \pause
  \vspace{\fill}

  Quizzes this semester will be a more frequent assessment of your understanding, with lower
  stakes. The intention is that they should not be nearly at the difficulty of examination questions
  or homeworks, but be a good way to demonstrate your understanding of the material so we can
  know early if anyone is falling behind.

  \pause
  \vspace{\fill}

  Fair game for quizzes is the \textbf{material covered in that week's lectures} and the
  \textbf{material graded in the previously due homework}. We will drop the lowest two
  quiz scores as well.

\end{frame}

\begin{frame}[fragile]
  \frametitle{AI Policy}

  Our AI policy is simple: \textbf{No usage of AI is permitted}.

  \pause
  \vspace{\fill}

  We will have more to say on this later. Suffice to say that you are at a
  stage of your education where you are not yet ready to use AI in a way that
  can help you learn.

  \pause
  \vspace{\fill}

  Please make maximal use of the humans that are being paid minimum wage (and me)
  to help you, whether through Piazza or office hours.
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{center}
      Good code should allow you to \textbf{reason without running}. \\
  ...
  \begin{center}
    \begin{minipage}{0.48\textwidth}
      \pause
      \begin{center}
      \includegraphics[scale=0.3]{goto.jpg}
      \end{center}
    \end{minipage}
    \begin{minipage}{0.48\textwidth}
      \begin{center}
      \includegraphics[scale=0.52]{goto.png}
      \end{center}
    \end{minipage}
  \end{center}
...
\begin{frame}[fragile]
  ...
  \begin{center}
    Good code should allow you to \textbf{reuse without rewriting}. \\

    \pause
    \vspace{\fill}

    Logically distinct parts should be \textit{modular}, for separate maintenance and reuse.
...
\begin{frame}[fragile]
  ...
  \begin{center}
    Good code should let you \textbf{change without breaking}. \\
  \end{center}
...
\begin{frame}[fragile]
  ...
  but knowing the \textbf{entire history of the program up until that point.}
  }
...
\begin{frame}[fragile]
  ...
  \defBox{\, The typing rule for function application is that \code{e1 e2 : t2} iff
   \code{e1 : t1 -> t2} and \code{e2 : t1}, for some types \code{t1}, \code{t2}.}
...
\sectionSlide{5}{Closing Remarks}

\begin{frame}[fragile]
  \frametitle{The AI-lephant in the room}
  ...
  We live in unprecedented times, where classrooms and software engineering are being
  disrupted by the rise of large language models and agentic coding assistants.

  \pause
  \vspace{\fill}

  It would be wrong of me as your instructor to not acknowledge this, and the fundamental
  question: \textbf{why are we here, learning functional programming?}

  \pause
  \vspace{\fill}

  What is the point of us learning how to write programs which can be generated by
  an LLM?
\end{frame}
...
\begin{frame}[fragile]
  ...
  How many of you are worried about having difficulty finding an entry-level job due
  to AI? About being replaced by AI?

  \pause
  \vspace{\fill}

  I cannot promise to you answers to all of these worries. What I can tell you is that
  there has never been a better time to really care about the material you
  have the opportunity to learn.

  \pause
  \vspace{\fill}

  In a world where generating code via LLMs is cheap, the ability to discern good solutions
  and reason about code is more valuable than ever.
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{minipage}[c]{0.55\textwidth}
    They\footnote{The New York Times, that is.} say that taste is the one thing that AI cannot replicate.

    \pause
    \vspace{10pt}

    \keyBox{ Generativity without constraint is chaos!}

    \pause
    \vspace{10pt}

    Without the ability to appraise the solutions that are presented to you, programming becomes a mess.

    \pause
    \vspace{10pt}

    You need to be able to choose what to do, and what to not do, and you cannot do that
    without first understanding the relative merits of different code.

  \end{minipage}%
  \hfill
  \begin{minipage}[c]{0.4\textwidth}
    \includegraphics[width=\textwidth]{taste.png}
  \end{minipage}
\end{frame}
...
\begin{frame}[fragile]
  ...
  % Some people ask "Why SML?"
  % Why functional programming?

  I promised you at the start of this lecture that I would change the way that you think.

  \pause
  \vspace{\fill}

  The point of 15-150 is not functional programming. Functional programming is a tool,
  a paradigm, and an art. But most of all, it is a way of thinking.

  \pause
  \vspace{\fill}

  Ideas from functional programming are not just pervasive among many disciplines of
  programming and in many programming languages, but they make you a stronger problem solver
  and better architect of code.

  \pause
  \vspace{\fill}

  You are at Carnegie Mellon University, an institution that is special in teaching you a subject that most never
  get the chance to learn.

  \pause
  \vspace{\fill}

  \textbf{I intend to keep my promise.}
\end{frame}
