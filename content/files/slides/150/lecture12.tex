...
\begin{frame}[fragile]
  ...
  \spec
    ...
    and \code{SOME (n div d)} otherwise}
...
\begin{frame}[fragile]
  ...
  \pause
  \begin{codeblock}
    fun reportGrades (records : (string * int) list) =
      (let
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item \code{2 handle Div => 3} $\hookrightarrow$ \code{2}
  \end{itemize}
...
\begin{frame}[fragile]
  ...
  \code{f}, as previously described, we define \code{f_opt : t1 -> t2 option} as:
  \begin{codeblock}
