...
\begin{frame}[fragile]
  ...
  Let's write a library for dictionaries\footnote{For pedagogical reasons,
  for the rest of this lecture we will use the type variable \code{'a}
  (when pertinent) to refer to the type of keys in the dictionary, and
  \code{'b} to refer to the type of values in the dictionary.}.
  ...
  \begin{codeblock}
    signature STR_DICT =
      sig
        ...
        val empty : 'b t
        val insert : key * 'b -> 'b t -> 'b t
        val lookup : key -> 'b t -> 'b option
      end
  \end{codeblock}
\end{frame}
...
\begin{frame}[fragile]
  ...
  Note that we chose to opaquely ascribe to the \code{STR_DICT} signature.
...
\begin{frame}[fragile]
  ...
  This looks better. We use the same familiar pattern of parameterizing our
  ...
  \begin{codeblock}
    signature POLY_DICT =
      sig
        ...
        (* mapping keys of type Key.t to values of 'b *)
        type `'b` t

        val empty : 'b t
        val insert : `Key.t` * 'b -> 'b t -> 'b t
        val lookup : `Key.t` -> 'b t -> 'b option
      end
  \end{codeblock}
  }
...
\begin{frame}[fragile]
  ...
  \begin{mdframed}[leftmargin=0.8cm, roundcorner=10pt, backgroundcolor=black!86!white, rightmargin=0.8cm]
  \begin{center}
  \includegraphics[scale=0.43]{using_functor}
  \end{center}
  \end{mdframed}
...
\begin{frame}[fragile]
  ...
  type of \code{'b t}, as we wanted, but it also hides the type of \code{Key.t}! There's nothing
  here which tells us that \code{Key.t} is \code{string}\footnote{Also a reminder that
  we are using only \code{'b} instead of \code{'a}, as we are only polymorphic
  in the type of values, not keys.}.
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{mdframed}[leftmargin=1.3cm, roundcorner=10pt, backgroundcolor=black!86!white, rightmargin=1.3cm]
  \begin{center}
  \includegraphics[scale=0.4]{using_functor2}
  \end{center}
  \end{mdframed}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{codeblock}
    `functor MkDict (Key : ORD)` :>
        POLY_DICT where type Key.t = `Key.t` =
      struct
        structure Key = `Key`
  ...
  Note that here we replaced our monomorphized type of \code{Key.t} from
  the hard-coded \code{string} to the \code{Key.t} type provided in the input
  structure.
\end{frame}
