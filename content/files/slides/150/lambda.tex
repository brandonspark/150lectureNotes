...
\begin{frame}[fragile]
  ...
  its \term{syntax} and \term{semantics}.

  \pause
  \vspace{\fill}

  \defBox{}{\, The syntax of a programming language is how you write
  ...
  \defBox{}{\, The semantics of a programming language is set of rules
...
\begin{frame}[fragile]
  ...
  \begin{align*}
    \nt{dec} & \stepsTo \gline{\underline{\nt{dec_1} \nt{dec_2}}} & \tag{SEQ} \\
              ...
              & \stepsTo \gline{val x = 1 val \_ = fn f => \underline{f} \underline{x}} & \tag{ID (x2)} \\
  \end{align*}
...
\begin{frame}[fragile]
  ...
  On the left, we see the judgements that are are allowed to use, in order
  to derive facts.
...
\begin{frame}[fragile]
  ...
  \[
    \infer[\rname{Add}]{e_1 \op{+} e_2 : \ty{int}}{e_1 : \ty{int} \quad e_2 : \ty{int}}
    \qquad
    \infer[\rname{Mul}]{e_1 \op{*} e_2 : \ty{int}}{e_1 : \ty{int} \quad e_2 : \ty{int}}
    \qquad
    \infer[\rname{Lit}]{n : \ty{int}}{}
  \]
  \[
    \infer[\rname{Tup}]{(e_1, e_2) : \ty{t_1} \op{*} \ty{t_2}}{e_1 : \ty{t_1} \quad e_2 : \ty{t_2}}
  \]

  \pause
  \vspace{\fill}

  We note that the \rname{Lit} judgement has no premises, because our
...
\begin{frame}[fragile]
  ...
  \[
    \infer[\rname{Mul}]{(1 \op{+} 2) \op{*} (3 \op{+} 4) : \ty{int}}{
      \infer[\rname{Add}]{1 \op{+} 2 : \ty{int}}{
        \infer[\rname{Lit}]{1 : \ty{int}}{} \qquad
        \infer[\rname{Lit}]{2 : \ty{int}}{}
      }
      \qquad
      \infer[\rname{Add}]{3 \op{+} 4 : \ty{int}}{
        \infer[\rname{Lit}]{3 : \ty{int}}{} \qquad
        \infer[\rname{Lit}]{4 : \ty{int}}{}
      }
    }
  \]
...
\begin{frame}[fragile]
  ...
  to the value $v$'':
  \[
    \infer[\rname{Lit}]{n \Downarrow n}{}
    \qquad
    \infer[\rname{Add}]{e_1 \op{+} e_2 \Downarrow n}
                       {e_1 \Downarrow n_1 \quad e_2 \Downarrow n_2 \quad n = n_1 + n_2}
    \qquad
    \infer[\rname{Tuple}]{(e_1, e_2) \Downarrow (v_1, v_2)}
...
\begin{frame}[fragile]
  ...
  \[
    \infer[\rname{Tuple}]{(1 + 2, 3 + 4) \Downarrow (3, 7)}{
      \infer[\rname{Add}]{1 + 2 \Downarrow 3}{
        \infer[\rname{Lit}]{1 \Downarrow 1}{} \quad
        \infer[\rname{Lit}]{2 \Downarrow 2}{}
      } \quad
      \infer[\rname{Add}]{3 + 4 \Downarrow 7}{
        \infer[\rname{Lit}]{3 \Downarrow 3}{} \quad
        \infer[\rname{Lit}]{4 \Downarrow 4}{}
      }
    }
  \]
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item \textbf{Case 1: \rname{Lit}} \\
    Here, $e = n$ and $v = n$. By the statics rule \rname{Lit}, $n : \ty{int}$,
    so $\ty{t} = \ty{int}$ and indeed $v : \ty{t}$. \\
    \item \textbf{Case 2: \rname{Add}} \\
    Here, $e = e_1 \op{+} e_2$ and $v = n$. The only statics rule for
    $e_1 \op{+} e_2$ is \rname{Add}, so $\ty{t} = \ty{int}$; and $n : \ty{int}$
    by \rname{Lit}. So $v : \ty{t}$.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    Then by \rname{Tup}, $(v_1, v_2) : \ty{t_1} \op{*} \ty{t_2} = \ty{t}$, as desired.
  \end{itemize}
\end{frame}

\sectionSlide{3}{The Lambda Calculus}
...
\begin{frame}[fragile]
  ...
  relation of \term{$\beta$-reduction}.
  \vspace{-15pt}
  \[
  ...
  \[
    ...
    variable capture. I will decline to elaborate further here.}}{}
  \]
...
\begin{frame}[fragile]
  ...
  The point being that, although we do not have numbers inbuilt in
...
\begin{frame}[fragile]
  ...
  { \large
  \begin{itemize}
    \item $\overline{0} \triangleq \lam{f} \lam{x} x$\footnote{Saying that $\overline{0} \triangleq M$ here means
    ...
    \item $\overline{1} \triangleq \lam{f} \lam{x} f x$
    \item $\overline{2} \triangleq \lam{f} \lam{x} f (f x)$
  \end{itemize}
  }
...
\begin{frame}[fragile]
  ...
  {\large
  $$\texttt{ADD} \triangleq \lambda n. \lambda m. \lambda f. \lambda x. n f (m f x)$$
  }

  \pause
  \vspace{\fill}

  Why does this work? We could induct to prove it, but essentially we see that
  ...
  Additionally, the definition of $\overline{n}$ is that $\overline{n} f x$ applies
  $f$ to $x$ $n$ times, and similarly for $\overline{m}$. Therefore
  $n f (m f x)$ applies $f$ to $x$ $n + m$ times, as desired.
\end{frame}
...
\begin{frame}[fragile]
  ...
  {\large
  $$\texttt{MUL} \triangleq \lambda n. \lambda m. \lambda f. \lambda x. n (m f) x$$
  }

  \pause
  \vspace{\fill}

  \customBox{Check your understanding}{\, Why does this work?}
...
\begin{frame}[fragile]
  ...
  arithmetic. There are also encodings which let us define
  complex data like tuples, control flow like conditionals, etc.

  \pause
  \vspace{\fill}

  This is cool, but not very useful without any repetition. The
  lambda calculus is only equipped with anonymous functions, meaning
  that we don't have any way to do recursion. Without recursion or
  loops, control flow isn't so useful.
...
\begin{frame}[fragile]
  ...
  Let's call the $ (\lambda x. f (x x)) (\lambda x. f (x x)) $ expression $ \texttt{SELF}(f) $. Then,
  replacing it in the lines above:
  \begin{align*}
    \Y \; f &\equiv_\beta (\texttt{SELF}(f)) & \tag{via def of \texttt{SELF}(f)}\\
                 &\equiv_\beta f (\texttt{SELF}(f)) & \tag{via def of \texttt{SELF}(f)}\\
                 &\equiv_\beta f (\Y \; f) & \tag{via def of \Y}\\
  \end{align*}
\end{frame}
...
\begin{frame}[fragile]
  ...
  $\Y \; f = f (\Y \; f)$. But how do we use this?

  \pause
  \vspace{\fill}

  Any function which is recursive is essentially just a function
  which has itself in scope. What if we write our desired-to-be-recursive
  ...
  {\large
  $$\texttt{fact}_{proto} \triangleq \lambda fact. \lambda n. \ldots$$
  }
\end{frame}
...
\begin{frame}[fragile]
  ...
  \texttt{IF} and \texttt{EQ} terms such that:
  \begin{itemize}
    \item \texttt{IF} $e_1$ $e_2$ $e_3$ evaluates to $e_2$ if $e_1$ evaluates to $\overline{true}$, and $e_3$ otherwise.
    \item \texttt{EQ} $e_1$ $e_2$ evaluates to $\overline{true}$ if $e_1$ and $e_2$ evaluate to the same Church numeral, and $\overline{false}$ otherwise.
    \item \texttt{PRED} $\overline{n + 1}$ evaluates to the Church numeral $\overline{n}$.
  \end{itemize}
  ...
  {\large
  $$\texttt{fact}_{proto} \triangleq \lambda fact. \lambda n.
  \texttt{IF} \pcond{(\texttt{EQ} \; n \; \overline{0})} \;
  \pbase{\overline{1}} \;
  \precase{(\texttt{MUL} \; n \; (fact \; (\texttt{PRED} \; n)))}$$
  }
  ...
  \begin{itemize}
    \item $\pcond{(\texttt{EQ} \; n \; \overline{0})}$ is the conditional, which
    checks if $n$ is equivalent to $\overline{0}$.
    \item $\pbase{\overline{1}}$ is evaluated to if the conditional succeeds
    \item $\precase{(\texttt{MUL} \; n \; (fact \; (\texttt{PRED} \; n)))}$ is the "else" of the
...
\begin{frame}[fragile]
  ...
  {\large
  $$\texttt{fact}_{proto} \triangleq \lambda fact. \lambda n.
  \texttt{IF} (\texttt{EQ} \; n \; \overline{0}) \;
  \overline{1} \;
  (\texttt{MUL} \; n \; (fact \; (\texttt{PRED} \; n)))$$
  }
  ...
  function, the lambda term such that $\texttt{fact} \; \overline{n}$ evaluates
  to $\overline{n!}$.
  ...
  the "true" $fact$, then $fact_{proto} \; \texttt{fact}$ would just actually
  be $\texttt{fact}$. So $\texttt{fact} = fact_{proto} \; \texttt{fact}$.
\end{frame}
...
\begin{frame}[fragile]
  ...
  Let's play around a bit. What happens if we evaluate $\Y \; \texttt{fact}_{proto}$?

  {\large
  \begin{align*}
    \Y \; \texttt{fact}_{proto}
    &\equiv_\beta \texttt{fact}_{proto} \; (\Y \; \texttt{fact}_{proto}) \\
    &\equiv_\beta \lambda n.\ \texttt{IF} \; (\texttt{EQ} \; n \; \overline{0}) \;
       \overline{1} \; (\texttt{MUL} \; n \; ((\Y \; \texttt{fact}_{proto}) \; (\texttt{PRED} \; n)))
  \end{align*}
  }
  ...
  We reduce \texttt{fact}$_{proto}$ on its argument, substituting the
  $\Y \; \texttt{fact}_{proto}$ term for $fact$. We are left with a
  lambda term which has a recursive call to $\Y \; \texttt{fact}_{proto}$
...
\begin{frame}[fragile]
  ...
  Now feed it a number. Write $F \triangleq \Y \; \texttt{fact}_{proto}$
  (so $F \equiv_\beta \texttt{fact}_{proto} \; F$). Then:
  {\large
  \begin{align*}
    F \; \overline{2}
      &\equiv_\beta \cl{(\texttt{fact}_{proto} \; F)} \; \overline{2}
        & \text{(since $F \equiv_\beta \texttt{fact}_{proto} \; F$)} \\
      &\equiv_\beta \cl{\texttt{IF} \; (\texttt{EQ} \; \overline{2} \; \overline{0}) \;
        \overline{1} \; (\texttt{MUL} \; \overline{2} \; (F \; (\texttt{PRED} \; \overline{2})))} & \text{def of \texttt{fact}$_{proto}$} \\
      &\equiv_\beta \cl{\texttt{MUL} \; \overline{2} \; (F \; \overline{1})}
        & \text{($\texttt{EQ} \; \overline 2 \; \overline 0$ is $\overline{false}$)} \\
      &\equiv_\beta \texttt{MUL} \; \overline{2} \; (\cl{\texttt{MUL} \; \overline{1} \; (F \; \overline{0})}) \\
      &\equiv_\beta \texttt{MUL} \; \overline{2} \; (\texttt{MUL} \; \overline{1} \; \cl{\overline{1}})
        & \text{($\texttt{EQ} \; \overline 0 \; \overline 0$ is $\overline{true}$)} \\
  ...
  The recursion bottoms out because \texttt{IF} discards the recursive
  branch once $\texttt{EQ} \; n \; \overline{0}$ is true. We built recursion
...
\begin{frame}[fragile]
  ...
  It is worth noting that everything that we have disussed so far is
...
% It is a subfield of computer science that deals with the design, implementation, and analysis of programming languages.
