...
\begin{parts}
  ...
  \part[5]\hfill
    ...
    \begin{solutionorbox}[18em]
      ...
      By the Master Theorem (or by drawing the recursion tree: level $i$ has
      $2^i$ nodes each doing $O(1)$ work, and there are $\log n$ levels, giving
      $\sum_{i=0}^{\log n} 2^i = O(n)$), this solves to $W(n) = O(n)$.

      Note this meets the required bound: every node contributes $O(1)$ work
