...
% DOCUMENT CLASS AND PACKAGE USE
    ...
    \newcommand{\auxColor}{338f8f}     % the color of note boxes and stuff
    \newcommand{\presentColor}{683799} % the primary color of the slide borders
...
\begin{frame}[fragile]
  ...
  \begin{center}
  \includegraphics[width=0.65\textwidth]{prt.jpg}
  \end{center}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \noteBox{ Tuples evaluate from \textbf{left to right}\footnote{This would go without saying, but this is not true in certain programming languages invented by the French.}. So \code{(1 + 1, 2 + 2)} $\stepsTo$ \code{(2, 2 + 2)} $\stepsTo$ \code{(2, 4)}}.
\end{frame}
...
\begin{frame}[fragile]
  ...
  \makebox[\textwidth][c]{
    \begin{tikzpicture}
      \node[rectangle,draw, minimum width=2.5in, fill=auxColor] (r1) at (0,0) {[\code{2}/\code{x}]};
      \node[rectangle,draw, minimum width=2.5in, fill=lightgray] (r2) at (0,0.75) {\code{fn (y : int) => x + y}};
      \begin{pgfonlayer}{background}
        \node[rectangle,draw,thick,inner sep=6pt,fit=(r1)(r2)] {};
      \end{pgfonlayer}
    \end{tikzpicture}
  }
...
\begin{frame}[fragile]
  ...
  In other words, \textbf{binding preserves function} \term{purity}.
\end{frame}
...
\begin{frame}[fragile]
  ...
  The goal is that patterns should be used to \textit{match} the values that you are interested
  in.
...
\begin{frame}[fragile]
  ...
  however.
  \begin{codeblock}
    val 2 : int = (1, 2) (* doesn't typecheck! never runs *)
  \end{codeblock}
  ...
  on the binding
  \begin{codeblock}
    val 2 : int = 1 (* runs, doesn't match! exception raised *)
  \end{codeblock}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \customColorBox{red}{white}{Warning}{Do not do this.}
\end{frame}
...
\begin{frame}[fragile]
  ...
  The reason is that \textbf{reduction implies equivalence}.
\end{frame}
...
\begin{frame}[fragile]
  ...
  \makebox[\textwidth][c]{
    \begin{tikzpicture}[box/.style={shape=rectangle,draw=black,fill=gray!25}]
      \node[] (E1) at (-4.5,5) {\textellipsis};
      \node[box] (T1) at (-4.5,4) {\small \code{1+(1+2)}};

      \node[] (E2) at (-2,5) {\textellipsis};
      \node[box] (T2) at (-2,4) {\small \code{1+(2+1)}};

      \node[] (E3) at (0.5,5) {\textellipsis};
      \node[box] (T3) at (0.5,4) {\small \code{(1+1)+2}};

      \node[] (E4) at (3.5,5) {\textellipsis};
      \node[box] (T4) at (3.5,4) {\small \code{2+(1+1)}};

      \node[] (E5) at (6,5) {\textellipsis};
      \node[box] (T5) at (6,4) {\small \code{(1+2)+1}};

      \node[] (E6) at (8.5,5) {\textellipsis};
      \node[box] (T6) at (8.5,4) {\small \code{(2+1)+1}};

      \node[box] (M1) at (-1,2) {\small \code{1 + 3}};
      \node[box] (M2) at (2,2) {\small \code{2 + 2}};
      \node[box] (M3) at (5,2) {\small \code{3 + 1}};
      \node[box] (L) at (2,0) {\small \code{4}} ;
...
\begin{frame}[fragile]
  ...
  like \term{referential transparency}.
