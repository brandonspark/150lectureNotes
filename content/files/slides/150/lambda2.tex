...
\sectionSlide{0}{Lambda Calculus and Programming Language Theory}

\begin{frame}[fragile]
\frametitle{What is the Lambda Calculus?}

What is the lambda calculus, and why are we here to study it?

\pause
\vspace{\fill}

\begin{minipage}{0.6\textwidth}
  The lambda calculus is a formal system first introduced by
  Alonzo Church in the 1930s. It is a simple and elegant language that
  expresses \term{computation as evaluation}, meaning that the
  running of a lambda calculus "program" is simply the application of
  simple rewriting rules.

  \pause
  \vspace{10pt}

  At its core, the lambda calculus is a Turing-complete model of
  computation that has the core components of any programming language:
  functions, variables, and function application\footnote{"Application" here
  meaning "calling".}.
\end{minipage}
\hspace{\fill}
\begin{minipage}{0.35\textwidth}
  \begin{center}
  \includegraphics[width=0.8\textwidth]{alonzo.jpg}
  Alonzo Church, first of his name
  \end{center}
\end{minipage}
...
\begin{frame}[fragile]
  ...
  The \term{lambda calculus} is essentially
  the simplest possible programming language. The key observation
  is that we can model any programming language as
...
\begin{frame}[fragile]
  ...
  In addition to its interesting mathematical properties, an
  understanding of the lambda calculus is a useful tool for
  the field of programming language theory, where we can
  understand the behavior of programming languages
  in terms of their fundamental types and terms.

  \pause
  \vspace{\fill}

  We'll learn some basic concepts about the \term{untyped lambda calculus}
  to gain familiarity, then later extend it to the
  \term{simply-typed lambda calculus}, which we can mathematically
  analyze to prove certain properties about.
...
\begin{frame}[fragile]
  ...
  The lambda calculus has j very simple grammar:
  ...
  Here, you should read $\lambda x.e$ as \code{fn x => e} in SML,
