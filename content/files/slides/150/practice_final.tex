...
\header{Final Exam}{}{15-150: Principles of Functional Programming}
\footer{}{\thepage}{}
...
Name \rule{5cm}{0.4pt} \, Andrew ID \rule{3cm}{0.4pt} \, House \rule{3.5cm}{0.4pt}
...
\input{draft_split.tex}

\input{draft_samefringe.tex}
...
\begin{parts}
  ...
  \begin{EnvUplevel}
    Your \code{balanced} used \code{foldl}, which walks the list left to right ---
    its span is $O(n)$. If instead the parentheses are stored in a \code{paren
    Seq.t}, we can check balancedness \textbf{in parallel}, using \code{reduce}
    (see the \code{SEQUENCE} signature in the appendix) to combine chunks in a
    balanced tree of depth $O(\log n)$.

    The catch: \code{reduce} may split the sequence and combine the pieces in
    \textit{any} grouping, so we cannot carry a left-to-right running depth. Each
    chunk must summarize itself as a value that two chunks can combine
    associatively.
  \end{EnvUplevel}

  \part[4]\hfill

    Here is the summary we will use. A chunk is summarized by a pair
    \code{(c, d) : int * int}, where \code{c} is the number of \textbf{unmatched
    closes} in the chunk (each needing an open to its \textbf{left}), and \code{d}
    is the number of \textbf{unmatched opens} left dangling to its \textbf{right}.

    Give the summary of a single \code{L} and of a single \code{R}, and then
    complete \code{combine}, which summarizes the concatenation of two chunks:
    \begin{codeblock}
      fun single L = (* ? *)
        | single R = (* ? *)

      fun combine ((c1, d1), (c2, d2)) = (* ? *)
    \end{codeblock}

    \textbf{Hint:} When two chunks meet, the left chunk's \code{d1} dangling opens
    can match the right chunk's \code{c2} needed closes. How many match, and what
    is left over on each side?

    \begin{solutionorbox}[16em]
      \begin{codeblock}
        fun single L = (0, 1)     (* one dangling open *)
          | single R = (1, 0)     (* one unmatched close *)

        fun combine ((c1, d1), (c2, d2)) =
          let
            val matched = Int.min (d1, c2)
          in
            (c1 + (c2 - matched), (d1 - matched) + d2)
          end
      \end{codeblock}

      \code{matched = min (d1, c2)} opens from the left meet closes from the
      right and cancel. The right chunk's remaining unmatched closes
      \code{c2 - matched} join \code{c1}; the left chunk's remaining dangling
      opens \code{d1 - matched} join \code{d2}.
    \end{solutionorbox}

  \part[4]\hfill

    Using \code{single} and \code{combine}, implement \code{balancedPar} on a
    sequence.
    \spec
      {balancedPar}
      {\code{paren Seq.t -> bool}}
      {\code{true}}
      {\code{balancedPar S} $\eeq$ \code{true} iff the parentheses in \code{S},
      in order, are balanced.}

    \begin{solutionorbox}[12em]
      \begin{codeblock}
        fun balancedPar S =
          let
            val (c, d) = Seq.reduce combine (0, 0) (Seq.map single S)
          in
            c = 0 andalso d = 0
          end
      \end{codeblock}

      \code{map single} summarizes each element; \code{reduce combine} folds the
      summaries in a tree with identity \code{(0, 0)}; the whole sequence is
      balanced exactly when nothing is left unmatched on either side.
    \end{solutionorbox}

  \part[3]\hfill

    State the \textbf{span} of \code{balancedPar} on a sequence of length \code{n},
    and explain in one sentence why \code{combine} being \textbf{associative} is
    what makes this parallel approach correct.

    \begin{solutionorbox}[12em]
      \code{reduce} combines the \code{n} summaries in a balanced tree of depth
      $O(\log n)$, and each \code{combine} is $O(1)$, so the span is
      $O(\log n)$ (versus $O(n)$ for the \code{foldl} version).

      \code{combine} must be \textbf{associative} because \code{reduce} is free
      to split the sequence and group the combines in any tree shape; only if
      \code{combine} is associative do all groupings yield the same result, so
      the answer is well-defined regardless of how the work is parallelized.
    \end{solutionorbox}
\end{parts}
...
\input{draft_fill.tex}

\input{draft_fold2.tex}

\input{draft_memoize.tex}

\input{draft_rle.tex}

\input{draft_treeall_proof.tex}

\newpage
\titledquestion{Types and Values (Pool)}
\textbf{Types and Values (Pool)}

For each expression below, give its most general type and value (or NWT / no
value, as appropriate).

\begin{parts}
\input{draft_types_pool.tex}
\end{parts}

\newpage
\titledquestion{Conceptual (Pool)}
\textbf{Conceptual (Pool)}

\begin{parts}
\input{draft_conceptual_pool.tex}
\end{parts}
