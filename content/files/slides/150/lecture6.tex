...
% DOCUMENT CLASS AND PACKAGE USE
    ...
    \subtitle{On the complexity of time} % TODO
    \date{28 May 2026} % TODO
...
\begin{frame}[fragile]
  ...
  \centerline{
  \begin{minipage}{0.4\textwidth}
    \includegraphics[scale=0.3]{graph4}
    $f$ is greater than $g$
  \end{minipage}
  \begin{minipage}{0.4\textwidth}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    stepping an expression once \pause
    \item $O(\log n)$ - Binary searching an interval $n$ wide, finding an
    element in a binary search tree \pause
    \item $O(n)$ - Computing the length of a list, finding the last element
    of a list, summing the nodes of a tree \pause
    \item $O(n \log n)$ - Mergesort and quicksort \pause
...
\begin{frame}[fragile]
  ...
  But, we see that \code{rev} actually depends on \code{@}.
...
\begin{frame}[fragile]
  \frametitle{A \code{trev}-elation}
...
\begin{frame}[fragile]
  ...
  This came out of the fact that due to \term{purity}, functions always behave the same on the same arguments.
...
\questionBreak{5840 8607}

\quizBreak{\textlangle obfuscated\textrangle}

\sectionSlide{3}{Parallelism and Span}
...
\begin{frame}[fragile]
  ...
  % i think i will change this for i will race a student, and enlist a TA to help
\end{frame}
...
\begin{frame}[fragile]
  ...
  \defBox{}{\, We say that the \term{work} of a process is the time expended using a single processor. }

  \pause
  \vspace{\fill}

  \defBox{}{\, We say that the \term{span} of a process is the time expended using infinitely many processors. }
...
\begin{frame}[fragile]
  ...
  \noteBox{}{\, This is a general rule going forward! Whenever you see a tuple,
  when we are doing span computations, you may assume that the elements of the tuple
  are being evaluated \textbf{in parallel}. This is \textit{only} for tuples.}
...
\begin{frame}[fragile]
  \frametitle{ Two Tree Cases }

  The main two cases we will be concerned about, for binary trees, will be the
...
%     Post-lecture survey: \\
%     \vspace{5pt}
%     \includegraphics[scale=0.035]{qr_june1}
%   \end{center}
% \end{frame}

\thankyou

\lecturePassword{BIG-OOF}
