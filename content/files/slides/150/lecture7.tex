...
%   \begin{center}
%     \begin{tabular}{ c|c|c }
%     Complexity of \code{treesum} (nodes) & Work & Span \\
%     \hline & \\[-1.5ex]
%      Balanced & TBD & $O(\log n)$ \\ [0.5ex]
%     \hline & \\[-1.5ex]
%      Unbalanced & TBD & $O(n)$
%     \end{tabular}
%   \end{center}
...
%   % \begin{center}
%   %   \begin{tabular}{ c|c|c }
%   %   Complexity of \code{treesum} (depth) & Work & Span \\
%   %   \hline & \\[-1.5ex]
%   %    Balanced & TBD & TBD \\ [0.5ex]
%   %   \hline & \\[-1.5ex]
%   %    Unbalanced & TBD & TBD
%   %   \end{tabular}
%   % \end{center}
...
\begin{frame}[fragile]
  ...
  So in a balanced tree, $n \approx 2^d$\footnote{Specifically, $n = 2^{d + 1} - 1$.}, so our previous bound is $O(\log n) = O(\log (2^d)) = O(d)$.
...
\begin{frame}[fragile]
  ...
  \pause
  \vspace{\fill}
  Note that we haven't done the work of \code{treesum} yet.

  \vspace{\fill}
  We will suspend our discussion of \code{treesum} for a second,
  and move on to a new function, \code{inord} for that.
