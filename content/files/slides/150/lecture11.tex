...
\begin{frame}[fragile]
  ...
  Suppose we have a list of grades, and we want to add a curve to each,
  and then find the average. Assume
  \code{applyCurve : int -> int} bumps a single grade.\footnote{
    Also, our grades are 1, 2, and 3. Our students didn't study a lot.
  }
  ...
  We have to introduce a lambda, because we want to use \code{L} twice.
  We need both the sum and the length.
  ...
  we rely on evaluation order inside that last lambda. Can we do better?
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{codeblock}
    [1, 2, 3]
    |> `(fn grades =>` map applyCurve `grades`
    |> (fn L =>   foldr op+ 0 L
...
\begin{frame}[fragile]
  \frametitle{Super Explicit Piping: Alternatively}

  \badBox{Subtle: Why did we not say that our pipeline is equivalent to
  this?}
  ...
  \textbf{Answer:} Here, each step closes its own lambda. So when we reach
  \code{List.length L}, the value \code{L} is \textbf{out of scope}
  -- it lived only inside the lambda two steps up! Nesting keeps \code{L}
  alive for the rest of the chain; this flat form throws it away.
\end{frame}

\begin{frame}[fragile]
  \frametitle{Super Explicit Piping: Alternatively}

  Put more clearly, we could have written this:
  ...
  But we chose instead to write this:
  ...
  The former is \code{x} being piped into two lambdas, each of which
  apply \code{f} and \code{g} to their arguments, respectively. The
  latter is \code{x} being piped into a \textbf{single lambda} that
  applies \code{f} to its argument, and then applies \code{g} to that
  result.

  \pause
  \vspace{\fill}

  The latter, however, lets us use the value of \code{y} at the
  step where we compute \code{g z}.
\end{frame}

% \begin{frame}[fragile]
%   \frametitle{Super Explicit Piping: Alternatively}

%   We can derive this from first principles, if you trust the fact
%   that \code{x |> f |> g} is equivalent to \code{x |> (g o f)}.

%   \pause
%   \vspace{\fill}

%   Then the former, written explicitly, is just
%   \code{x |> (fn y => f y) |> (fn z => g z)}.

%   \pause
%   \vspace{\fill}

%   Then the latter, written explicitly, is just
%   \begin{align*}
%     & \code{x |> (g o f)} \\
%     & \eeq \code{x |> (fn y => (g o f) y)} \tag{eta expansion} \\
%     & \eeq \code{x |> (fn y => g (f y))} \tag{definition of \code{o}} \\
%     & \eeq \code{x |> (fn y => f y |> g)} \tag{definition of \code{|>}} \\
%     & \eeq \code{x |> (fn y => f y |> (fn z => g z))} \tag{eta expansion} \\
%   \end{align*}
...
%   \begin{align*}
%     & \code{x |> (fn y => f y) |> (fn z => g z)} \\
%     & \eeq \code{x |> (fn y => f y |> (fn z => g z))} \\
%   \end{align*}
% \end{frame}

\begin{frame}[fragile]
  \frametitle{Super Explicit Piping: Indented}

  Actually, we usually prefer to indent it slightly differently:

  \begin{codeblock}
    [1, 2, 3]             |> (fn grades =>
    map applyCurve grades |> (fn L =>
  ...
  Now, each expression, such as \code{map applyCurve grades}, is placed
  on the same line as the lambda expression that it is being piped into,
  which explicitly names the result of the computation (in this case, \code{L}).
...
\begin{frame}[fragile]
  ...
  \begin{codeblock}[style=expandedHL]
    [1, 2, 3] |> <r(r>fn grades =>
      map applyCurve grades |> <p(p>fn L =>
        foldr op+ 0 L |> <g(g>fn total =>
...
\begin{frame}[fragile]
  ...
  \begin{codeblock}
    [1, 2, 3]                  |> (fn grades =>
    `mapCool` applyCurve grades  `  ` (fn L =>
...
\begin{frame}[fragile]
  ...
  \badBox{This is an incorrect implementation of \code{map}!}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{codeblock}
    fun tmap_backwards f [] acc = acc
      | tmap_backwards f (x::xs) acc =
          tmap_backwards f xs (f x :: acc)
...
\lecturePassword{THECONTROLMUSTFLOW}
