...
\begin{frame}[fragile]
  ...
  \begin{minipage}{0.6\textwidth}
    Joseph Marie Jacquard (1752-1834), inventor of the Jacquard loom.

    \vspace{10pt}

    Made-to-order and constructed via 24,0000 punched cards on a
    Jacquard loom. Charles Babbage owned one of these portraits,
    and was inspired by it to build the Analytical Engine.


  \end{minipage}
  \begin{minipage}{0.35\textwidth}
  \begin{center}
  \includegraphics[width=0.9\textwidth]{jacquard.jpg}
  \end{center}
  \end{minipage}
...
\begin{frame}[fragile]
  ...
  using punched cards for the purpose of data storage. This was later
  realized to be a stupid name, and it was renamed to the International Business Machines
  ...
  working, programmable electrical computer. Punched cards were still in use,
  however, to store the memory of the machine.
\end{frame}
...
\begin{frame}[fragile]
  ...
  maybe we don't need to write assembly by hand. Maybe we can write
  a program which can turn human-readable instructions into the assembly
  itself.
...
\begin{frame}[fragile]
  ...
  In a broad sense, we should have that \code{interpret} $\eeq$ \code{run o compile}\footnote{
    More readably, \code{interpret prog} $\eeq$ \code{run (compile prog)}
  }.
...
\begin{frame}[fragile]
  ...
  \footnote{
    This is a somewhat imperfect analogy, because you could argue
    that an interpreter does have a real-time translation equivalent.
    But it happens so quickly that for all intents and purposes, you could
    consider an interpreter to not have a true translated "middle ground".
  }
\end{frame}
...
\begin{frame}[fragile]
  ...
  the SML/NJ compiler is written in SML, and the Rust compiler is written in Rust\footnote{
    But before it was written in Rust, it was written in OCaml, which
    is essentially SML!
  }.
...
\begin{frame}[fragile]
  \frametitle{Programming Languages as Hammers}

  I find it is easiest to understand this by way of analogy.

  \pause
  \vspace{\fill}

  Suppose you are John W. Whackus, american carpenter scientist,
  and you are very enthusiastic about hand tools. You have an idea
  in your head of a hand tool with a weighted head that can be used to
  easily deliver precise impact.

  \pause
  \vspace{\fill}

  You call this a hammer, but unfortunately it doesn't exist yet--nobody
  has thought of it or built it before.

  \pause
  \vspace{\fill}

  Building the metallic head of the hammer requires the kind of precise
  blunt force that you would like to have a hammer for in the first place.
  But, nothing stops you from using a random rock instead, since hammers
  don't exist yet. The key is that once you use the rock to build the first
  hammer, you can use that hammer to build the next hammer, and so on.
\end{frame}
...
\begin{frame}[fragile]
  ...
  the entire process an absolute joy\footnote{If you recall how nice red-black trees
  are in SML, this is the same reason why compilers are nice in SML.}.
...
\begin{frame}[fragile]
  ...
  \begin{codeblock}[keywordstyle=, morekeywords={}, style=expandedHL]
...
\begin{frame}[fragile]
  ...
  We see it's just a straightforward recursive function on a tree\footnote{
    Type \textit{inference}, where we may not have complete information over
    what type our components are, takes a more complex algorithm. The
    typical way this is implemented is with a technique called Hindley-Milner
    type inference, and is beyond the scope of this lecture.
  }.
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{codeblock}
    fun cfold (e : exp) : exp =
      ...
      | Div   (Int i1, Int 0)  => e
      | Div   (Int i1, Int i2) => Int (i1 div i2)
      `| Plus  (e1, e2) => cfold (Plus  (cfold e1, cfold e2))`
      `| Minus (e1, e2) => cfold (Minus (cfold e1, cfold e2))`
      `| Div   (e1, e2) => cfold (Div   (cfold e1, cfold e2))`
      `(* ... many such cases ... *)`
  \end{codeblock}
  }
...
\begin{frame}[fragile]
  ...
  Then, we introduce additional unsafe behavior\footnote{Arguably,
  due to integer overflow, moving the multiplication was also
  problematic.}!
