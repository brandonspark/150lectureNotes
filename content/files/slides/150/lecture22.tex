...
\begin{frame}[plain]
  \makebox[\textwidth][c]{\begin{tikzpicture}[remember picture,overlay]
    ...
    \node at (0, -0.9) {\resizebox{\paperwidth}{!}{\includegraphics{lecture23fake.png}}};
...
\begin{frame}[fragile]
  ...
  the late Michael Erdmann, I'm going to do so by going backwards -- by starting at
...
\begin{frame}[fragile]
  \frametitle{Lecture 22: Program Analysis}
...
\begin{frame}[fragile]
  \frametitle{Lecture 21: Compilers}
...
{
\renewcommand{\auxColor}{08b1d4}     % the color of note boxes and stuff
\renewcommand{\presentColor}{6D4FE8} % the primary color of the slide borders
\renewcommand{\bgColor}{ece4f0}      % the color of the background of the slide
\renewcommand{\darkBg}{8b98ad}
\renewcommand{\lambdaColor}{\auxColor}
...
\begin{frame}[fragile]
  \frametitle{Lecture 20: Asynchronous Programming}

  We can model many different kinds of programming problems with
  functional principles--even ones traditionally understood to be
  "systems" or "low" level.

  \pause
  \vspace{\fill}

  We saw that we could expand our set of problem statements from purely
  mathematical, value-based computations to practical ones involving
  interaction with the outside world, without compromising our high-level
  abstractions and mathematical principles.

  \pause
  \vspace{\fill}

  We saw how to solve the same asynchronous problems in SML that JavaScript
  developers had to solve a decade ago, which ultimately used the same
  CPS and thunk machinery that we had already learned about.

  \pause
  \vspace{\fill}

  \lessonBox{\,
    Simple mathematical principles belie the real-world complexities that
    engineers have to work with every day.
  }
\end{frame}
}
...
\begin{frame}[fragile]
  ...
  nothing to do with actual impact\footnote{Tokens, anyone?}.
...
\begin{frame}[fragile]
  ...
  The educational landscape, along with every other landscape, for that matter,
  has been changing. I've felt that this semester.

  \pause
  \vspace{\fill}

  There's a very justifiable sense of fear for those of you who are
  studying, who are being let out into an uncertain world. What is the
  point of functional programming, when the future of software engineering
  and the job market are so in question?

  \pause
  \vspace{\fill}

  My two cents--I do not believe that computer science adjacent fields are
  inherently doomed. It will be hard, but there have always been things
  that are hard in your path to get here. We are living in a time where
  the low bar to enter the field makes your knowledge and understanding
  even more valuable.

  \pause
  \vspace{\fill}

  What matters most is that you are genuinely interested in the things
  you learn, that you are persevering, and that you don't lose track
  of the higher level lessons of this course.
\end{frame}
...
\begin{frame}[fragile]
  ...
  Plus, at the very least, HackerNews thinks functional programming
  is cooler than AI.

  \vspace{\fill}

  \begin{center}
  \includegraphics[width=0.9\textwidth]{ceo.JPG}
  \end{center}
...
\begin{frame}[fragile]
  ...
  This iteration of 15-150 is dedicated to the memory of Michael Erdmann.

  \pause
  \vspace{\fill}

  I TA'd under Mike for many years, and he has undoubtedly shaped the
  way that I teach this course. His kindness and willingness to go
  far for his TAs and students is what I remember the most.

  \pause
  \vspace{\fill}

  I share this excerpt from his speech from winning the Herbert A. Simon
  Teaching Award in 2024, "The Enduring Connectedness of Learning":

  \pause
  \vspace{5pt}

  \begin{quote}
    The best information is likely online not on one teacher's chalkboard.
    Still, two themes remain: Education enables and learning connects.
    Our students are here to improve themselves and their lives.
    And despite instant electronic connectivity, in the evening our buildings
    are filled with students, working together, trying to understand each
    other's thinking, helping each other past difficulties. That too I find
    inspiring.
  \end{quote}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{center}
    ...
    the years of teaching this class.

    \pause
    \vspace{\fill}

    My friends in Pittsburgh who supported and kept me sane throughout this summer.

    \pause
    \vspace{\fill}

    My friends back home who I left to be here.

    \pause
    \vspace{\fill}

    Dilsun Kaynar, for her pivotal support in the administration of this course.

    \pause
    \vspace{\fill}

    Semgrep, for supporting my decision in coming here to teach.
  \end{center}
\end{frame}
...
\begin{frame}[fragile]
  ...
  you, no tricks, no jokes, completely unplanned, because... what are you looking at? Fuck.
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Beginnings and Endings}

  I started school at Carnegie Mellon in the fall of 2018.

  \pause
  \vspace{\fill}

  Over four years, I TA'd the course, then returned to teach it as
  an instructor in the summer of 2023.

  \pause
  \vspace{\fill}

  Though not all at the same time, my journey with 150 has spanned an
  eight year period--nearly a decade since I first set foot on this campus.

  \pause
  \vspace{\fill}

  The last people I really knew on campus, the class that started the year
  after I graduated, have now just graduated.
...
\begin{frame}[fragile]
  ...
  instead go to industry, and leave academia behind me.


  \pause
  \vspace{\fill}

  In 2023, I was allowed to teach this course over the summer, and I
  embraced it wholeheartedly as the last I would see of 150. I made my
  peace with that.
...
\begin{frame}[fragile]
  ...
  I will probably never teach again\footnote{I mean it this time.}. But that's OK. Teaching all of you has been
