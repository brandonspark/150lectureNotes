...
% DOCUMENT CLASS AND PACKAGE USE
    ...
    \newcommand{\auxColor}{18b6d6}     % the color of note boxes and stuff
...
\begin{frame}[fragile]
  ...
  Our solution was to simply not play, by only dealing with \term{pure} code
...
\begin{frame}[fragile]
  ...
  We've shied away from it thus far, but at some point we have to be able to read
...
\begin{frame}[fragile]
  ...
  {\color{blue}\href{https://www.infoq.com/presentations/Null-References-The-Billion-Dollar-Mistake-Tony-Hoare/}{"billion-dollar mistake"}}\footnote{His words, not mine.}\footnote{Who actually died earlier this year. Rest in peace.}
...
\begin{frame}[fragile]
  ...
  This is because \code{r2} is of type \code{string ref}, and
...
\begin{frame}[fragile]
  ...
  \begin{center}
    ...
    \begin{minipage}{0.15\textwidth}
      \centering
      \begin{tikzpicture}
        \node[box, label=below:\code{r1}] (N1) {\code{2}};
      \end{tikzpicture}
    \end{minipage}
    \begin{minipage}{0.1\textwidth}
      \centering
  ...
  We simply unpack the contents of the box \code{r1}, which is \code{2}.
\end{frame}
...
\begin{frame}[fragile]
  ...
  For instance, for the refs \code{r1 : int ref} and \code{r2 : string ref}
  ...
  of pattern matching includes an \textit{implicit dereference}, because we
...
\begin{frame}[fragile]
  \frametitle{Too Many \code{ref}s}

  We need to be careful not to overdo it, though! Our problem with the
  last function was that it had too few \code{ref}s, but it is also
  possible to have too \textit{many}.
  ...
  This code is wrong, because every single call to \code{fact}, including
  its recursive calls, will spawn a new \code{store} that no one else
  uses. This means we always return \code{n}.

  \pause
  \vspace{\fill}

  Our next idea will be to use a single private \code{ref} cell for each
  top-level call to \code{fact}, but to use an inner helper to do the
  actual recursion.
\end{frame}
